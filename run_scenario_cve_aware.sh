#!/bin/bash
#
# run_scenario_cve_aware.sh - Automated script to run the CVE-aware worm scenario
#
# This script is identical to run_scenario.sh but uses the CVE-aware worm instead
#
# Usage: ./run_scenario_cve_aware.sh [options]
#
# Options:
...
step4_compile_worm() {
    ...
    # Compile all source files with static linking
...
step6_run_exploit() {
    ...
    if [ $INTERACTIVE -eq 1 ]; then
        ...
        echo "  2. Download and execute the CVE-aware worm from C2"
        echo "  3. CVE-aware worm will propagate to spoke servers"
        echo ""
    ...
    log_info "Command: Download and execute CVE-aware worm from C2"
    ...
    if [ $EXPLOIT_RESULT -eq 0 ]; then
        ...
        log_info "CVE-aware worm should now be running in pms_server memory"
...
step7_verify() {
    ...
    log_info "Waiting 30 seconds for CVE-aware worm to propagate to spoke servers..."
    log_info "The worm needs time to:"
    log_info "  1. Start executing on pms_server"
    log_info "  2. Run risk assessment"
    log_info "  3. Discover SSH keys and networks"
    log_info "  4. Scan and infect spoke servers"
    sleep 30
...
main() {
    echo ""
    echo -e "${BOLD}${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}CVE-Aware Worm Scenario - Automated Runner${NC}              ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
