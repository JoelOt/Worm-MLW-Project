#!/bin/bash
#
# setup_environment_cve_aware.sh - Environment preparation script for CVE-aware worm propagation scenario
#
# This script sets up the environment by:
# 1. Generating SSH keys
# 2. Building Docker images
# 3. Starting containers
# 4. Compiling the CVE-aware worm binary
# 5. Building the fileless dropper
#
# Usage: ./setup_environment_cve_aware.sh [options]
#
# Options:
...
# Step 4: Compile CVE-aware worm
step4_compile_worm() {
    log_step "Step 4: Compiling CVE-Aware Worm (x86_64 Linux)"
    
    if [ $SKIP_COMPILE -eq 1 ]; then
        log_warn "Skipping worm compilation (--skip-compile flag)"
        if [ ! -f "./worm/CVE_aware/build/worm_cve_aware" ]; then
            log_error "CVE-aware worm binary not found! Cannot continue without compilation."
            exit 1
        fi
        # Verify it's x86_64 Linux binary
        if command -v file &> /dev/null; then
            FILE_OUTPUT=$(file ./worm/CVE_aware/build/worm_cve_aware 2>/dev/null || echo "")
    ...
    log_info "Compiling CVE-aware worm binary in x86_64 Linux container..."
    ...
    # Compile all source files with static linking using Makefile
    docker run --rm --platform linux/amd64 \
        -v "$PWD":/src -w /src \
        gcc:13 \
        bash -c "make clean && make CC=gcc CFLAGS='-Wall -Wextra -std=c11 -static -s -O2 -pipe -Wno-unused-function' LDFLAGS='-static'" 2>&1 | tail -20
    
    if [ ! -f "build/worm_cve_aware" ]; then
    ...
    if command -v file &> /dev/null; then
        FILE_OUTPUT=$(file build/worm_cve_aware 2>/dev/null || echo "")
        if echo "$FILE_OUTPUT" | grep -q "x86-64\|x86_64"; then
    ...
    WORM_SIZE=$(stat -f%z build/worm_cve_aware 2>/dev/null || stat -c%s build/worm_cve_aware 2>/dev/null)
    log_success "CVE-aware worm compiled successfully (${WORM_SIZE} bytes, x86_64 Linux)"
...
step5_build_dropper() {
    ...
    # Copy CVE-aware worm binary
    if [ ! -f "../../worm/CVE_aware/build/worm_cve_aware" ]; then
        log_error "CVE-aware worm binary not found! Run compilation first."
        cd ../..
        exit 1
    fi
    
    log_info "Copying CVE-aware worm binary..."
    cp ../../worm/CVE_aware/build/worm_cve_aware ./worm
    
    if [ ! -f "worm" ]; then
        log_error "Failed to copy worm binary!"
        cd ../..
        exit 1
    fi
...
get_display_name() {
    ...
    case "$container" in
        ...
        "local_site_4") echo "LOCAL SITE 4 (High-Risk - Self-Destruct)" ;;
        *)              echo "$container" ;;
    esac
}
...
open_interactive_logs() {
    ...
    containers=("attacker" "c2" "pms_server" "local_site_1" "local_site_2" "local_site_3" "local_site_4")
...
SCRIPTEOF
            ...
            if osascript -e "tell application \"Terminal\" to do script \"$temp_script\"" > /dev/null 2>&1; then
                log_info "Opened log window for ${container}"
            else
                log_warn "Failed to open log window for ${container}"
            fi
            
            # Longer delay to avoid overwhelming the system and ensure windows open properly
            sleep 0.5
        done
    else
...
main() {
    echo ""
    echo -e "${BOLD}${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}Environment Setup - CVE-Aware Worm Propagation Scenario${NC}    ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo ""
    log_info "Next steps:"
    echo "  ./run_scenario_cve_aware.sh          - Run the full CVE-aware worm scenario"
    echo "  ./run_scenario_cve_aware.sh --interactive - Run scenario with confirmation prompt"
    echo ""
